...
class Medium():
    ...
    def create_filter(self): 
        ...
        val_cal = max(self._medium)
        S       = np.fft.fft(self._medium)
        S[0]    = 0+0*1j
        OF_med  = np.fft.ifft(S*S.conjugate()/self._NPS).real
        K       = val_cal/np.amax(OF_med)  
        H = K*S.conjugate()/self._NPS   #H è la funzione di trasferimento del filtro, nel dominio delle frequenze

        return H
    
    def PSD(edata,t_sample,wind = "hann"): 
        '''
    ...
    def parabolic_fit(self,mat,stop=0):
    ...
    def applyOF(self,edata,H):
        ...
        a,b,c       = self.parabolic_fit(events_OF)[0:3]
        _   = np.argmax(events_OF,axis=1)
